...
if __name__ == "__main__":
    ...
    model_name = "model0.h5" # DONT FORGET TO CHANGE THIS
    ...
    models[0][class_object - 1] = get_q_network(weights_path + model_name)
...
#         print(image_names1.shape, image_names2.shape)
        image_names = np.concatenate([image_names1, image_names2], axis=1)
    else:
        image_names = np.array([load_images_names_in_data_set('trainval', path_voc)]) # SINCE WE"RE SHORT ON TIME
#         image_names = np.array([load_images_names_in_data_set('trainval', path_voc2)])
    ...
    for i in range(epochs_id, epochs_id + epochs):
        print("Epoch:", i)
        for j in tqdm(range(0, np.size(image_names))):
            masked = 0
            not_finished = 1
            image = images[j].astype("float32")
            image_name = image_names[0][j]
            if two_databases == 1:
                if j < np.size(image_names1): #??? wtf is going here
            ...
            for k in range(np.size(array_classes_gt_objects)):
                ...
                if array_classes_gt_objects[k] == class_object:
                    ...
                    iou, new_iou, last_matrix, index = follow_iou(gt_masks, region_mask, array_classes_gt_objects,
                                                                  class_object, last_matrix, available_objects, region_image)
                    new_iou = iou
                    ...
                    while (status == 1) & (step < number_of_steps) & not_finished:
                        ...
                        if action == 6:
                            iou, new_iou, last_matrix, index = follow_iou(gt_masks, region_mask,
                                                                          ...
                                                                          last_matrix, available_objects, region_image)
                            gt_mask = gt_masks[:, :, index]
